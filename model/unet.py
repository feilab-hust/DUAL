...
class UNet(nn.Module):
    def __init__(
        ...
        print('padding size: {}, padding_mode: {}'.format(self.padding_size, self.padding_mode))
    ...
    def _padding_input(self, x, padding_size, padding_mode):
        total_stride = 2 ** (len(self.channel_mults) - 1)
        pad = (padding_size // 2, padding_size - padding_size // 2,
               padding_size//2, padding_size - padding_size // 2)
        x = F.pad(x, pad, padding_mode)
        assert x.shape[2] % total_stride == 0 and x.shape[3] % total_stride == 0, f'invalid padding size: {padding_size}'
        return x

    def _crop_output(self, x, padding_size):
        left, right, top, bottom = (padding_size // 2, padding_size - padding_size // 2,
                                    padding_size // 2, padding_size - padding_size // 2)
    ...
    def forward(self, x , time=None):
        ...
        if self.padding_size > 0:
            x = self._padding_input(x, self.padding_size, self.padding_mode)
        feats = [x]
        ...
        if self.padding_size > 0:
            noise = self._crop_output(noise, self.padding_size)
        return noise
